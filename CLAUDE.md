...
Метод Init каждого сервиса и контроллера обязан быть идемпотентным: повторный вызов не создаёт вторых подписок на события, не запускает второй цикл и не дублирует состояние. Проверять флагом инициализации в начале Init и выходить, если он уже поднят.

Весь текст, который видит игрок, пишется только на английском. Это касается всего без исключения: интерфейса, надписей на табличках и объектах сцены, названий зон и кнопок, счётчиков, считалок, сообщений о кике и ошибках, реплик боссов, уведомлений и всего, что появится позже. Комментарии в коде, сообщения коммитов и общение со мной остаются на русском — правило только про то, что видно игроку на экране.
